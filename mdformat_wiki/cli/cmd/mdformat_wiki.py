"""Implements ``mdformat_wiki`` commands."""
...
from mdformat_wiki.cli.utils import command
...
def version(app: typer.Typer) -> typer.Typer:
    ...
    def _version_callback(value: bool) -> None:
        """Print the installed ``mdformat-wiki`` version and exit."""
        if value:
            typer.echo(importlib.metadata.version('mdformat-wiki'))
            raise typer.Exit()
    ...
    def _main(version: Optional[bool] = version) -> None:
        """Mdformat Wiki command-line interface."""
...
def install(app: typer.Typer) -> typer.Typer:
    ...
    def _install(
        ...
        """Install the mdformat_wiki skill for Claude Code and Codex.
        ...
        base = importlib.resources.files('mdformat_wiki')
